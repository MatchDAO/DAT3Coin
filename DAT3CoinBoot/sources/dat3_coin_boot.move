...
module dat3::dat3_coin_boot {
    ...
    public entry fun initializeWithResourceAccount(aptospadAdmin: &signer, metadata: vector<u8>, byteCode: vector<u8>) {
        assert!(signer::address_of(aptospadAdmin) == @dat3_admin, ERR_PERMISSIONS);
        let (resourceSigner, resourceSignerCap) = account::create_resource_account(aptospadAdmin, b"dat");
